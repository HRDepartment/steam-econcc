describe("EconCC TF", function () {
    var EconCC = require('../index.js');
    var currencies = require('./currencies.json');

    var ec, f;
    var ecparams = EconCC.converters.backpackTF(currencies);
    beforeEach(function () {
    ...
    describe("#format", function () {
        it("formats usd", function () {
            ...
            expect(f('100 usd:Long')).toBe('$100.00 (869.57 ref, 51.15 keys)');
        });
        ...
        it("formats metal", function () {
            ...
            expect(f('100 metal:Long')).toBe('100.00 ref (5.88 keys, $11.50)');
        });
        ...
        it("formats hat", function () {
            ...
            expect(f('100 hat:Long')).toBe('100 hats (133.00 ref, 7.82 keys, $15.29)');
        });
        ...
        it("formats keys", function () {
            ...
            expect(f('100 keys:Long')).toBe('100 keys (1,700.00 ref, $195.50)');
        });
        ...
        it("uses step", function () {
            ...
            expect(f('100 hat:Long')).toBe('100 hats (133.00 ref, 7.8 keys, $15.29)');

            ec.scope({step: EconCC.Disabled}, function () {
                expect(f('1.48 keys', EconCC.Mode.Label)).toBe('1.48 keys');
            });
        });

        describe("Mode = Label", function () {
            it("selects the most expensive currency", function () {
                expect(f('15 usd:Label')).toBe('7.67 keys');
            ...
            it("ignores currencies when cur.label = false", function () {
                ec.currencies.earbuds.hidden = ec.currencies.earbuds.label = false;
    ...
    describe("#parse", function () {
        ...
        it("parses double dash", function () {
            expect(ec.parse("$1.--")).toEqual({low: 1, currency: 'usd', high: undefined, mode: undefined});
        });
    });
    ...
    describe("#scm", function () {
        ...
        it("calculates the SCM seller price", function () {
            ...
            expect(ec.scm({low: 2.49, currency: 'usd'}).seller).toEqual({value: 2.18, currency: 'usd'});
            expect(ec.scm({low: 2.50, currency: 'usd'}).seller).toEqual({value: 2.19, currency: 'usd'});
        });
        it("reverse calcs with mode = SCM.Seller", function () {
            expect(ec.scm({low: 0.01, currency: 'usd'}, EconCC.SCM.Seller).buyer).toEqual({value: 0.03, currency: 'usd'});
            expect(ec.scm({low: 2.18, currency: 'usd'}, EconCC.SCM.Seller).buyer).toEqual({value: 2.49, currency: 'usd'});
            expect(ec.scm({low: 2.19, currency: 'usd'}, EconCC.SCM.Seller).buyer).toEqual({value: 2.50, currency: 'usd'});
        });
    });
